...
function repeatString(str, times) {
  return str.length !== 0 && times > 0 ? str.repeat(times) : '';
}
...
function removeFirstOccurrences(str, value) {
  const i = str.includes(value) ? str.indexOf(value) : str;
...
function removeLastOccurrences(str, value) {
  const i = str.includes(value) ? str.lastIndexOf(value) : str;
  return str.slice(0, i) + str.slice(i + value.length);
}
...
function isPalindrome(str) {
  const newStr = str
    ...
    .reverse()
    .join('');
  const origStr = str.toLowerCase().replace(/[^a-z]/g, '');
...
function findLongestWord(sentence) {
  ...
  return arr.reduce(
    (acc, word) => (acc.length < word.length ? word : acc),
    firstWord
  );
}
